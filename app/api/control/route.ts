...
async function saveTaskRecurrence({ client, user }: RequestContext, body: Record<string, unknown>) {
  ...
  if (["interval", "weekly"].includes(recurrence.frequency) && !taskDueDate) {
...
function recurrenceOptions(body: Record<string, unknown>): RecurrenceOptions | Response {
  ...
  if (["interval", "weekly"].includes(frequency) && deadlineMode !== "scheduled") {
  ...
  return {
    ...
    intervalValue,
    intervalUnit,
    endsOn: ["interval", "weekly"].includes(frequency) ? endsOn : null,
    businessDaysOnly,
    nightShiftPattern,
    nightShiftOnDays,
    nightShiftOffDays,
