...
type FormState = { id?: string; entryDate: string; medication: MedicationPreset; otherMedication: string; dosage: string; effect: string };

const blankForm = (): FormState => ({ entryDate: localToday(), medication: "Ritalin", otherMedication: "", dosage: "", effect: "" });
...
function MedicationRecords() {
  ...
  function edit(record: MedicationRecord) {
    ...
    setForm({ id: record.id, entryDate: record.entry_date, medication: known ? record.medication as MedicationPreset : "其他藥物", otherMedication: known ? "" : record.medication, dosage: record.dosage, effect: record.effect ?? "" });
  ...
  return <div className="space-y-5 medication-print-root">
    ...
    <form className="panel p-4 sm:p-5 no-print" onSubmit={save}>
      <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-bold">{form.id ? "修改紀錄" : "新增藥物紀錄"}</h2><p className="muted mt-1 text-sm">日期預設今日，你可直接更改。</p></div>{form.id ? <Button type="button" variant="ghost" onClick={() => setForm(blankForm())}>取消修改</Button> : null}</div>
      <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <label><span className="label">日期</span><input className="field mt-2" type="date" value={form.entryDate} onChange={(event) => setForm({ ...form, entryDate: event.target.value })} required /></label>
        <label><span className="label">藥物</span><select className="field mt-2" value={form.medication} onChange={(event) => setForm({ ...form, medication: event.target.value as MedicationPreset })}>{medicationPresets.map((item) => <option key={item} value={item}>{item}</option>)}</select></label>
        {form.medication === "其他藥物" ? <label><span className="label">其他藥物名稱</span><input className="field mt-2" value={form.otherMedication} onChange={(event) => setForm({ ...form, otherMedication: event.target.value })} placeholder="自行輸入藥物名稱" required /></label> : <div className="hidden lg:block" />}
        <label><span className="label">劑量</span><div className="relative mt-2"><input className="field pr-12" value={form.dosage} onChange={(event) => setForm({ ...form, dosage: event.target.value })} placeholder="例如 10" required /><span className="pointer-events-none absolute inset-y-0 right-4 grid place-items-center text-sm font-bold text-slate-500">mg</span></div></label>
      </div>
      <label className="mt-4 block"><span className="label">效果</span><textarea className="field mt-2 min-h-24" value={form.effect} onChange={(event) => setForm({ ...form, effect: event.target.value })} placeholder="自由填寫；可留空。" /></label>
      {message ? <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm font-semibold text-amber-900" role="alert">{message}</p> : null}
      <Button className="mt-4" type="submit" disabled={saving}>{saving ? "儲存中…" : <><Plus className="h-5 w-5" />{form.id ? "儲存修改" : "儲存紀錄"}</>}</Button>
    </form>
