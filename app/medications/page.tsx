...
type MedicationRecord = { id: string; entry_date: string; taken_time: string | null; medication: string; dosage: string; effect: string | null; created_at: string; updated_at: string };
type MedicationDraft = { takenTime: string; medication: MedicationPreset; otherMedication: string; dosage: string; effect: string };
type FormState = { id?: string; entryDate: string; entries: MedicationDraft[] };

const blankEntry = (): MedicationDraft => ({ takenTime: "", medication: "Ritalin", otherMedication: "", dosage: "", effect: "" });
...
function MedicationRecords() {
  ...
  function edit(record: MedicationRecord) {
    ...
    setForm({ id: record.id, entryDate: record.entry_date, entries: [{ takenTime: record.taken_time?.slice(0, 5) ?? "", medication: known ? record.medication as MedicationPreset : "其他藥物", otherMedication: known ? "" : record.medication, dosage: record.dosage, effect: record.effect ?? "" }] });
  ...
  return <div className="space-y-5 medication-print-root">
    ...
    <form className="panel p-4 sm:p-5 no-print" onSubmit={save}>
      <div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-bold">{form.id ? "修改紀錄" : "新增藥物紀錄"}</h2><p className="muted mt-1 text-sm">日期預設今日；每隻藥可分別填寫實際服用時間。</p></div>{form.id ? <Button type="button" variant="ghost" onClick={() => setForm(blankForm())}>取消修改</Button> : null}</div>
      ...
      <div className="mt-4 space-y-4">
        {form.entries.map((entry, index) => <fieldset key={index} className="rounded-xl border border-slate-200 bg-slate-50/70 p-4"><div className="flex items-center justify-between gap-3"><legend className="text-sm font-extrabold text-slate-800">藥物 {index + 1}</legend>{!form.id && form.entries.length > 1 ? <button type="button" className="text-sm font-bold text-slate-600 underline" onClick={() => setForm({ ...form, entries: form.entries.filter((_, entryIndex) => entryIndex !== index) })}>移除此藥</button> : null}</div><div className="mt-3 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <label><span className="label">實際服用時間（可留空）</span><input className="field mt-2" type="time" value={entry.takenTime} onChange={(event) => updateEntry(index, { takenTime: event.target.value })} /></label>
