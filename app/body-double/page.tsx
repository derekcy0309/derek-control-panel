...
function SessionRoom({ data, session, busy, online, now, onPrepare, onStart, onPresence, onComplete, onCancel }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {session.status === "running" && selfTask && self && !["completed", "left"].includes(self.status) ? <CompletionCheckpoint task={selfTask} busy={busy} timeFinished={timeFinished} onComplete={onComplete} /> : null}
...
function CompletionCheckpoint({ task, busy, timeFinished, onComplete }: { task: BodyDoubleTaskOption; busy: boolean; timeFinished: boolean; onComplete: () => Promise<void> }) {
