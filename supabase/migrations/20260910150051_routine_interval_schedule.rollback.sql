-- Export or remove interval routines before rollback. Rolling back while such
-- rules exist would discard their cadence and ending policy.
do $$
begin
  if exists (
    select 1 from public.task_recurrence_rules where frequency = 'interval'
  ) then
    raise exception 'ROUTINE_INTERVAL_ROLLBACK_REQUIRES_EXPLICIT_DATA_HANDLING';
  end if;
end $$;
...
begin
  ...
  if rule.deadline_mode = 'none' then
    ...
    perform private.enqueue_notification(
      rule.owner_id, 'recurrence_reminder', 'task', next_task_id, reminder_at,
...
drop function if exists private.next_task_interval_date(date, integer, text, boolean);
