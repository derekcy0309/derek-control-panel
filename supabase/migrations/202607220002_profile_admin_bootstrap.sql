-- Permit tightly scoped admin-flag bootstrapping from the trusted postgres role.
...
where lower(p.display_name) = lower('Kwok_cy')
  and exists (select 1 from public.tasks t where t.owner_id = p.user_id);
