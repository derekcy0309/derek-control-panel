-- Restores the claim routine from the notification-system migration. This is
-- intentionally exact historical behaviour; it also restores the known
-- PL/pgSQL ambiguity, so use only when rolling the whole notification release
-- back to its previous database definition.
