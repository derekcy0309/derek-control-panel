...
const emptyData: AppData = { tasks: [], transactions: [], meetings: [], balances: [], settings: null };
...
export function useAppData() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData({ tasks: result.tasks, transactions: result.transactions, meetings: result.meetings, balances: result.balances, settings: result.settings });
    } catch (caught) {
