...
export function RoleDailyDashboard({
  ...
  const topCandidates = uniqueTasks(topTasks).filter(todayEligible).slice(0, 3);
    ...
    <section className="space-y-4" aria-label={workspaceRoleLabels[role]}>
      ...
      {urgent.length ? (
        <DashboardSection title="真正緊急或已逾期" count={urgent.length} tone="urgent">
          {urgent.map((task) => <CompactTaskCard key={task.id} task={task} />)}
        </DashboardSection>
      ) : null}
...
function SukiDailyFollowup({ data, summary }: { data: TodayData; summary: ReturnType<typeof buildSukiFollowupSummary> }) {
  ...
  return (
    <section className="rounded-2xl border border-indigo-100 bg-indigo-50/50 p-4 sm:p-5">
      <div className="flex items-start gap-3"><BellRing className="mt-0.5 h-5 w-5 text-indigo-600" /><div><h3 className="font-extrabold text-slate-950">今日綜合提醒</h3><p className="mt-1 text-sm text-slate-600">接近期限、等待別人及需要重新安排的工作集中一次查看；非緊急提醒每天只會合併顯示一至兩次。</p></div></div>
